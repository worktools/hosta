...
import { json, error, appById, deploymentById, sha, randomBytes } from "../utils.js";
...
export function registerDeploymentRoutes(
  ...
  const deploymentGetMatch = url.pathname.match(/^\/api\/apps\/([^/]+)\/deployment$/);
  if (method === "GET" && deploymentGetMatch) {
    ...
    if (!deployment) return json(res, 200, { appId: app.id, status: "none" }), true;
    const version = deployment.versionId ? store.versions.find((v) => v.id === deployment.versionId) : null;
    json(res, 200, {
      id: deployment.id, appId: deployment.appId, status: deployment.status, versionId: deployment.versionId,
      versionNumber: version?.number || null, versionStatus: version?.status || null,
  ...
  if (method === "POST" && rollbackMatch) {
    (async () => {
      ...
      const prevVersions = store.versions
        .filter((v) => v.appId === app.id && v.status === "ready" && v.id !== deployment.versionId)
        .sort((a, b) => b.number - a.number);
      const prevVersion = prevVersions[0];
      if (!prevVersion) return error(res, 400, "NO_ROLLBACK", "No previous version to roll back to");
      deployment.versionId = prevVersion.id;
      deployment.updatedAt = new Date().toISOString();
      app.publishedVersionId = prevVersion.id;
      app.updatedAt = new Date().toISOString();
      await save();
      json(res, 200, { deploymentId: deployment.id, versionId: prevVersion.id, versionNumber: prevVersion.number });
    })().catch((e) => error(res, 500, "INTERNAL_ERROR", e.message));
    return true;
  }

  // POST /api/deployments/:id/unpublish
  const unpublishMatch = url.pathname.match(/^\/api\/deployments\/([^/]+)\/unpublish$/);
  ...
  const regenKeyMatch = url.pathname.match(/^\/api\/deployments\/([^/]+)\/regenerate-key$/);
  if (method === "POST" && regenKeyMatch) {
    (async () => {
      ...
      deployment.updatedAt = new Date().toISOString();
      await save();
      json(res, 200, { deploymentId: deployment.id, webhookKey, webhookUrl: `/hooks/${deployment.id}` });
    })().catch((e) => error(res, 500, "INTERNAL_ERROR", e.message));
    return true;
  }
