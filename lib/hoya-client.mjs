...
export async function engineStatus() {
  try {
    ...
    if (!response.ok || !Array.isArray(data?.protocolVersions) || !data.protocolVersions.includes('1') || data.wasmAbi !== 'hoya-json-v1' || !Array.isArray(data.runtimes) || !['javascript', 'wasm'].every(r => data.runtimes.includes(r))) throw new EngineError('ENGINE_INCOMPATIBLE', 'Hoya execution v1 with JS and WASM is required');
...
export async function executeWithHoya(version, input, runId, datasource = null) {
  ...
  const request = { protocolVersion: '1', runId, runtime, code: version.code, artifactSha256: hash, input, datasource, limits, capabilities: { network: [] } };
  let response;
  ...
  if (!data || typeof data !== 'object' || data.protocolVersion !== '1' || data.runId !== runId || data.artifactSha256 !== hash || !statuses.includes(data.status) || !Array.isArray(data.logs) || !Number.isFinite(data.metrics?.durationMs) || data.metrics.durationMs < 0 || !Object.hasOwn(data, 'result') || (data.status === 'succeeded' ? data.error !== null : typeof data.error?.code !== 'string' || typeof data.error?.message !== 'string' || typeof data.error?.retryable !== 'boolean')) throw new EngineError('ENGINE_PROTOCOL', 'Hoya returned a mismatched or invalid execution response');
  ...
  if (data.logs.some(log => !log || typeof log.level !== 'string' || typeof log.message !== 'string' || !Number.isFinite(log.at) || !Number.isFinite(new Date(log.at).getTime()))) throw new EngineError('ENGINE_PROTOCOL', 'Hoya returned invalid structured logs');
  // Only protocol fields may cross into the control-plane run record.
  return { protocolVersion: data.protocolVersion, runId: data.runId, artifactSha256: data.artifactSha256,
    status: data.status, result: data.result, logs: data.logs.map(({ level, message, fields, at }) => ({ level, message, fields, at })),
    metrics: { durationMs: data.metrics.durationMs },
    error: data.error === null ? null : { code: data.error.code, message: data.error.message, retryable: data.error.retryable } };
}
