...
function diagnosticsFor(code) {
  const diagnostics = [];
  const policyError = validateCode(code);
  if (policyError) diagnostics.push({ severity: 'error', code: 'POLICY_REJECTED', message: policyError });
    ...
    if (!/ctx\.log\s*\(/.test(code)) diagnostics.push({ severity: 'info', code: 'NO_STRUCTURED_LOGS', message: '建议在关键分支调用 ctx.log，便于定位线上输入问题。' });
...
async function deepSeekGenerate({ description, sampleInput }) {
...
async function execute(version, input, trigger) {
  ...
  try {
    const policyError = validateCode(version.code); if (policyError) { run.status = 'rejected'; run.error = { code: 'POLICY_REJECTED', message: policyError }; return run; }
    const logs = [];
...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'POST' && url.pathname === '/api/apps') {
      ...
      const app = { id: id('app'), name: String(input.name).trim().slice(0, 120), description: String(input.description).trim().slice(0, 4000), sampleInput: input.sampleInput ?? {}, draftVersionId: null, publishedVersionId: null, createdAt: now(), updatedAt: now() };
    ...
    if (req.method === 'POST' && versionCreateMatch) {
      ...
      const diagnostics = diagnosticsFor(code); const hasError = diagnostics.some((item) => item.severity === 'error');
      const version = { id: id('ver'), appId: app.id, number: store.versions.filter((v) => v.appId === app.id).length + 1, status: hasError ? 'needs_revision' : 'ready', source: 'manual-edit', summary: String(payload.summary || '用户编辑的程序版本').slice(0, 500), code, codeSha256: sha(code), tests: [], validationError: diagnostics.find((item) => item.severity === 'error')?.message || null, diagnostics, createdAt: now() };
    ...
    if (req.method === 'POST' && generateMatch) {
      ...
      const generated = await deepSeekGenerate({ description: app.description, sampleInput: app.sampleInput }); const problem = validateCode(generated.code);
      const version = { id: id('ver'), appId: app.id, number: store.versions.filter((v) => v.appId === app.id).length + 1, status: problem ? 'needs_revision' : 'ready', source: generated.source, summary: generated.summary, code: generated.code, codeSha256: sha(generated.code), tests: generated.tests, validationError: problem || null, createdAt: now() };
    ...
    if (req.method === 'POST' && diagnoseMatch) {
      const version = versionById(diagnoseMatch[1]); if (!version) return error(res, 404, 'NOT_FOUND', 'Version not found'); const input = await body(req); const diagnostics = diagnosticsFor(version.code); const run = diagnostics.some((item) => item.severity === 'error') ? null : await execute(version, input, 'diagnostic');
